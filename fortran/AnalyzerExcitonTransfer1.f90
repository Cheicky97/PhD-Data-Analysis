...
PROGRAM Analyzer
 ...
 IMPLICIT NONE
    ...
    WRITE(9,*) 'Before using on other molecules, please check the compatibility&
            of the used algorithm with your system(molecule)'
    WRITE(9,*) '                                     Version 1.1  &
       '
    WRITE(9,*) '***********************************************************************&
            ****************************************'
    ...
    DO WHILE (bool)
        ...
        IF (TRIM(control) == '&LENGTHENING') THEN
            ...
            DO WHILE (bool2)
                ...
                IF (TRIM(control) == '}') bool2 = .FALSE.
            ENDDO
            CALL lenghtening(SUM(monomer),SUM(monomer) +&
                    INT((nbOfCycleOut-monomer(1)))*(n_s + n_c + n_h),&
        ...
        IF (TRIM(control) == '&STACKING') THEN
            DO WHILE (bool2)
                ...
                IF (TRIM(control) == 'ALTERNATE') READ(*,*) alternate
                IF (TRIM(control) == 'PERIODICITY') READ(*,*) stack
                IF (TRIM(control) == '}') bool2 = .FALSE.
            ENDDO
            CALL stackMonomer(SUM(monomer),SUM(monomer)*Nb_stack,TRIM(inputFile),LEN(TRIM(inputFile)),Nb_stack,stack,&
